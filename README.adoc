...
In this section you will create a simple Spring Boot REST service which accepts external configuration from the Spring Cloud Configuration service.
...
Add the Spring Cloud dependencies. The `spring-cloud-pivotal-connector` specifically enables support for automatically discovering a running Spring Cloud Configuration Server service.
...
Push the JAR file to Cloud Foundry and name the new app `restclient`.

  cf push rest-client -p target/gs-configuration-server-client-0.1.0.jar

Bind the Configuration Server service to the newly published app.

  cf bind-service rest-client config-server

Restage the rest-client to ensure the changes take effect
...
Make a request to the `rest-client` app:

  curl http://rest-client.{PCF DOMAIN}
